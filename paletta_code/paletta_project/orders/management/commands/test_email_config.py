...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if current_backend == 'django.core.mail.backends.console.EmailBackend':
            self.stdout.write(self.style.WARNING('⚠️  WARNING: You are using the console email backend!'))
            ...
            if not getattr(settings, 'AWS_SES_ENABLED', False):
                self.stdout.write(self.style.WARNING('⚠️  AWS_SES_ENABLED is False'))
                
            if not getattr(settings, 'AWS_ACCESS_KEY_ID', ''):
                self.stdout.write(self.style.WARNING('⚠️  AWS_ACCESS_KEY_ID not set'))
                
            if not getattr(settings, 'AWS_SECRET_ACCESS_KEY', ''):
                self.stdout.write(self.style.WARNING('⚠️  AWS_SECRET_ACCESS_KEY not set'))
            ...
            if not getattr(settings, 'EMAIL_HOST', ''):
                self.stdout.write(self.style.WARNING('⚠️  EMAIL_HOST not set'))
                
            if not getattr(settings, 'EMAIL_HOST_USER', ''):
                self.stdout.write(self.style.WARNING('⚠️  EMAIL_HOST_USER not set'))
                
            if not getattr(settings, 'EMAIL_HOST_PASSWORD', ''):
                self.stdout.write(self.style.WARNING('⚠️  EMAIL_HOST_PASSWORD not set'))
